...
// Hi?n th? 1 dòng giá tr? ? m?i bu?c: tt, S, (w,path[1]),................(w,path[n]))
  void displayStep(int step, boolean [] selected, int [] dist, int [] path,int p, int [] sele, int nSele, boolean [] stopDisplay)
   {int i;//selected[]:là 0,1 dánh d?u d?nh dã du?c dua vào trong m?ng S
    String S = "";
    for(i=0;i<nSele;i++) S = S + b[sele[i]];
// M?ng b tên c?a các d?nh (ABC...)
// b[sele[i]]:ch? tên d?nh t?i ch? s? sele[i]:cho là ch? s? c?a m?i pt dã du?c duy?t trong b
    System.out.printf("\n\n %2d:  %10s",step,S);
    for(i=0;i<n;i++)
     {if(i==p) continue;
      if(dist[i]==INF)
