...
public class ArticleDAO {
     ...
     * getListArticle.<br>
     * 
     * Lấy tất cả thông tin của các món Sushi trong database đồng thời xử lí việc 
     * phân trang qua 2 biến pageSize(tổng số bài viết trên 1 trang) và pageIndex
     * (trang hiện tại).
    ...
    public List<Article> getListArticle(int pageIndex, int pageSize) throws SQLException {
        ...
        try {
            String sql = "SELECT * FROM(SELECT *,ROW_NUMBER() OVER (ORDER BY ID ASC) as row_num FROM  Article) tblArticle "
     ...
     * countAllResult. <br>
     * 
     * Đếm số bản ghi của bảng Article để phân trang dựa trên tổng số bài viết trên
     * 1 trang và trang hiện tại.
     * Sau khi thực hiện xong câu lệnh đóng kết nối.
     *
     * @return a number
     * @throws SQLException
